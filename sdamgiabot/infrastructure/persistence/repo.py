...
class UserRepository(AbstractUserRepository):
    ...
    def set_raw_user_preferred_topics(
            self,
            user_id: UserId,
            topics: str
    ) -> None:
    ...
    def get_seen_tasks(self, user_id: UserId) -> list[tuple]:
        self._ensure_user_exists(user_id)
        seen_str = User.get(User.tg_id == user_id).already_seen
        return [(x[:x.find(":")], x[x.find(":") + 1:]) for x in
                seen_str.split(";")]
    ...
    def has_seen_task(self, user_id: UserId, task: AbstractTask) -> bool:
        ...
        seen_str = User.get(User.tg_id == user_id).already_seen
        seen = seen_str.split(";")
        return f"{task.subject.uid}:{task.uid}" in seen

    def get_user_ids_for_daily_task(self) -> list[UserId]:
        return [user.tg_id for user in
                User.select().where(User.receive_daily == True)]
